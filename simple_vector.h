...
template < typename Type>
	class SimpleVector
	{
		public: using Iterator = Type*;
		using ConstIterator = const Type*;

		SimpleVector() noexcept: size_(0),
			capacity_(0),
			arr_(nullptr) {}
		...
		SimpleVector(ReserveProxyObj obj): size_(0),
			capacity_(obj.capacity),
			arr_(static_cast<size_t> (0))
			{
			}
		...
		void Resize(size_t new_capacity)
		{
			if (new_capacity > capacity_)
			{
                ...
                std::generate(new_arr_.Get() + size_, new_arr_.Get() + new_capacity, [] () {return Type();}); // Initialize new elements with default value
		...
		SimpleVector(const SimpleVector &other): size_(other.GetSize()),
